  ...
  loadData() {
    ...
    .subscribe(res => {
      ...
      for (let i = 0; i < accounts.length; i++) {
        ...
        account.orders_1h = account.orders.filter((o: Date) => o >= new Date(Date.now() - 1 * 60 * 60 * 1000)).sort((a: Date, b: Date) => a.getTime() - b.getTime());
        account.orders_2h = account.orders.filter((o: Date) => o >= new Date(Date.now() - 2 * 60 * 60 * 1000)).sort((a: Date, b: Date) => a.getTime() - b.getTime());
        account.orders_4h = account.orders.filter((o: Date) => o >= new Date(Date.now() - 4 * 60 * 60 * 1000)).sort((a: Date, b: Date) => a.getTime() - b.getTime());
        account.orders_12h = account.orders.filter((o: Date) => o >= new Date(Date.now() - 12 * 60 * 60 * 1000)).sort((a: Date, b: Date) => a.getTime() - b.getTime());
        account.orders_24h = account.orders.filter((o: Date) => o >= new Date(Date.now() - 24 * 60 * 60 * 1000)).sort((a: Date, b: Date) => a.getTime() - b.getTime());
